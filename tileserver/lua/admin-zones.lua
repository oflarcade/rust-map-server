...
local function create_zone()
    ...
    -- Validate constituent pcodes belong to this tenant's scope
    local pcodes_literal = "{" .. table.concat(constituent_pcodes, ",") .. "}"
    local valid_check, err0 = pg.exec([[
    ...
    if valid_count ~= #constituent_pcodes then
        ...
        ngx.say(cjson.encode({
            error  = "One or more constituent_pcodes are not in this tenant's scope",
            code   = "INVALID_PCODES",
    ...
    local insert_result, err1 = pg.exec([[
        WITH next_num AS (
            SELECT COALESCE(MAX(
    ...
    ]], {parent_pcode, tenant_id, zone_name, color, pcodes_literal})
