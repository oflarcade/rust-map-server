...
STATES_GADM_DIR="$GADM_DIR/states"
FILTER_SCRIPT="$BASE_DIR/scripts/filter-gadm.py"
...
GADM_FILE="$GADM_DIR/${COUNTRY}_2.json"
...
# Verify GADM file
if [ ! -f "$GADM_FILE" ]; then
    log_error "GADM file not found: $GADM_FILE"
    log_info "Run ./scripts/setup.sh to download GADM data"
    exit 1
fi
...
if [ "$LIST_MODE" = true ]; then
    echo ""
    log_info "Available states for ${COUNTRY}:"
    echo ""
    python3 -c "
import json
...
if [ ${#STATES[@]} -eq 0 ]; then
    log_info "No states specified - auto-discovering all states from GADM..."
...
# Create directories
mkdir -p "$OUTPUT_DIR" "$BOUNDARIES_DIR" "$TEMP_DIR" "$DATA_SOURCES_DIR" "$STATES_GADM_DIR"
...
echo ""
log_step "Step 1: Filtering GADM boundary data for selected states..."

python3 "$FILTER_SCRIPT" "$GADM_FILE" "$STATES_GADM_DIR" "${STATES[@]}"

BOUNDS_FILE="$STATES_GADM_DIR/bounds.json"
if [ ! -f "$BOUNDS_FILE" ]; then
    log_error "Bounds file not generated"
    exit 1
fi

log_success "GADM data filtered successfully"
...
print(data['$SLUG']['name'])
")
        GADM_STATE_FILE="$STATES_GADM_DIR/${SLUG}.json"
        BOUNDARY_OUTPUT="$BOUNDARIES_DIR/${COUNTRY}-${SLUG}-admin.pmtiles"

        if [ ! -f "$GADM_STATE_FILE" ]; then
            log_warn "No GADM file for $STATE_NAME, skipping boundaries"
            continue
        fi
    ...
    COMBINED_GADM="$STATES_GADM_DIR/combined.json"
    COMBINED_BOUNDARY="$BOUNDARIES_DIR/${COUNTRY}-states-admin.pmtiles"

    if [ -f "$COMBINED_GADM" ]; then
