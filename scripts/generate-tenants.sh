...
for DEF in "${FILTERED_DEFS[@]}"; do
    ...
    # -- COMBINED MULTI-STATE --
        ...
        GEOJSON_FILE="$GADM_DIR/states/${SLUGS}.json"
        if [ ! -f "$GEOJSON_FILE" ]; then
            log_error "GeoJSON file missing: $GEOJSON_FILE - generate it first"
...
"
    ...
    # -- SINGLE STATE --
        ...
        for STATE in "${STATE_ARRAY[@]}"; do
            ...
            GEOJSON_FILE="$GADM_DIR/states/${SLUG}.json"
            if [ ! -f "$GEOJSON_FILE" ]; then
                log_error "GeoJSON file missing: $GEOJSON_FILE - run filter-gadm.py first"
                FAILED+=("Tenant $T_ID ($T_NAME): missing GeoJSON file")
                continue
            fi
