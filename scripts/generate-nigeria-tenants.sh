...
OSM_FILE="$BASE_DIR/osm-data/nigeria-latest.osm.pbf"
...
GADM_FILE="$GADM_DIR/nigeria_2.json"
...
if [ ! -f "$GADM_FILE" ]; then
    ...
    log_info "Run ./scripts/download-gadm.sh first"
    exit 1
fi

mkdir -p "$OUTPUT_DIR" "$DATA_SOURCES_DIR" "$TEMP_DIR" "$STATES_GADM_DIR"
...
log_step "Step 1: Computing bounding boxes from GADM..."
...
python3 "$FILTER_SCRIPT" "$GADM_FILE" "$STATES_GADM_DIR" "${ALL_STATES[@]}"
...
min_lon = min(data[s.lower().replace(' ','-')]['min_lon'] for s in states)
min_lat = min(data[s.lower().replace(' ','-')]['min_lat'] for s in states)
max_lon = max(data[s.lower().replace(' ','-')]['max_lon'] for s in states)
max_lat = max(data[s.lower().replace(' ','-')]['max_lat'] for s in states)
