...
GADM GeoJSON (state level 2) → filter-gadm.py (Python)
                               → gadm/states/*.json
                                 → Planetiler with custom profile
                                   → <country>-admin.pmtiles (GADM boundaries)
```
...
- **Layer 2**: Boundary tiles from `<country>-admin.pmtiles` (GADM: district/LGA outlines)
...
- **GADM source files**: `gadm/<country>_0.json` (country), `gadm/<country>_2.json` (state/LGA level)
- **Filtered states**: `gadm/states/<country>-<state>.json` (generated by filter-gadm.py)
- **Metadata**: `gadm/states/<country>-<state>.bbox.json` (bounding box coordinates)
...
### State Name Matching
State names in `generate-states.ps1` must match GADM `NAME_1` property values **exactly** (case-sensitive). Example: "Lagos", "Edo", "Bayelsa" for Nigeria. Use `-List` flag to discover available state names:
```powershell
...
- **`scripts/filter-gadm.py`**: GADM filtering utility
  - Usage: `python3 filter-gadm.py <gadm_level2_file> <output_dir> <state1> <state2> ...`
  - Outputs: filtered GeoJSON files + `.bbox.json` metadata
...
- **Python 3**: GADM filtering scripts (check: `python3 --version`)
